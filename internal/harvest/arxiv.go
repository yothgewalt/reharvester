...
const (
	// https directly: the http URL answers with a redirect, which is a second
	// request against arXiv's rate limit for every page.
	...
	DefaultPageSize = 200
	// DefaultMax is the record cap Harvest applies when Query.Max is unset.
	DefaultMax = 2000
	// The paper retains records whose abstract exceeds 200 characters.
	MinAbstractChars = 200
)

// Client is the arXiv API source. Its HTTP, Delay and retry behaviour are also
// the polite transport every other networked source wraps; see Client.get.
type Client struct {
	HTTP *http.Client
	...
	Delay    time.Duration
	PageSize int
	// MaxRetries bounds retries of a throttled or timed-out request; zero
	// means 4. Router lowers it so a blocked source is abandoned quickly.
	MaxRetries int
...
//
// Keywords and years are split as splitHarvest describes. Keywords are never
// OR-ed into a single request: arXiv sorts by date, so the broadest term wins
// the whole cap. Harvesting "aerodynamic OR fighter jet OR space" as one query
// returns papers about latent spaces and nothing about aircraft.
func (c *Client) Harvest(ctx context.Context, q Query, prog Progress) ([]paper.Paper, error) {
	return splitHarvest(ctx, q, prog, tolerate(c.harvestSpan))
}

// harvestSpan pages one query to a record cap. seen may be shared across calls
// to deduplicate across years; a nil map means start fresh. On error it returns
// what it had collected alongside the error.
func (c *Client) harvestSpan(ctx context.Context, q Query, max int, seen map[string]struct{}, prog Progress) ([]paper.Paper, error) {
	...
	for start := 0; len(out) < max; start += pageSize {
		...
		page := make([]paper.Paper, 0, len(f.Entries))
		for _, e := range f.Entries {
			if p, ok := toPaper(e); ok {
				page = append(page, p)
			}
		}
		var kept int
...
// politeness delay; see Client.pause. Throttling is retried; see Client.get.
func (c *Client) fetch(ctx context.Context, v url.Values) (*feed, error) {
	body, err := c.get(ctx, c.base()+"?"+v.Encode(), nil)
	if err != nil {
		return nil, err
	}
	var f feed
...
func toPaper(e entry) (paper.Paper, bool) {
	...
	// everything arXiv serves is open access
	return newPaper(paper.StripVersion(e.ID), e.Title, e.Summary, authors, cats, published, true)
}
